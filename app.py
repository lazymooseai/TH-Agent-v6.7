...
def hae_tapahtumat_api(hakusana: str, pvm_iso: str) -> list:
    if not hakusana: return []
    
    dt = datetime.datetime.strptime(pvm_iso, "%Y-%m-%d")
    seuraava_paiva = (dt + datetime.timedelta(days=1)).strftime("%Y-%m-%d")
    
    # Huom! Pyydetään include=location, jotta näemme salin/näyttämön nimen tarkasti
    params = {
        "text": hakusana,
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            # Sanalista, jolla tunnistetaan pienet "ei-tuottavat" sivunäyttämöt
            pieni_keywords = [
                "pieni", "almin", "almisali", "studio pasila", "lilla teatern", 
                "arena-näyttämö", "camerata", "sonore", "black box", "organo", 
                "paavo", "klubi", "lämpiö", "aula", "kahvila", "ravintola"
            ]
            ...
            for t in r.json().get("data", []):
                ...
                # NÄYTTÄMÖN TUNNISTUS
                loc = t.get("location", {})
                loc_name = loc.get("name", {}).get("fi", "").lower() if isinstance(loc, dict) else ""
                
                extra = t.get("location_extra_info", {})
                extra_str = extra.get("fi", "").lower() if isinstance(extra, dict) else str(extra or "").lower()
                
                hall_text = f"{loc_name} {extra_str} {nimi.lower()}"
                
                is_pieni = any(kw in hall_text for kw in pieni_keywords)
                
                # Sallitaan sama kellonaika VAIKKA olisi tuplabuukkaus, JOS toinen on päänäyttämö ja toinen pieni
                avain = f"{nimi}-{alku_klo}-{is_pieni}"
                ...
                if is_pieni:
                    # Harmaa teksti ja pienempi fontti piilotetuille/huonoille kohteille
                    tulos_pieni.append(f"<div style='color:#777; font-size:15px; margin-bottom:4px; line-height:1.2;'>▷ Klo {aika_naytto}: {nimi} (Sivunäyttämö)</div>")
                else:
                    # Kirkas keltainen korostus rahaa tuoville päänäyttämöille
                    tulos_paa.append(f"<div style='color:#d0d0d0; font-size:18px; margin-bottom:6px; line-height:1.3;'>► <b>Klo {aika_naytto}</b>: {nimi} <span style='color:#ffeb3b; font-weight:bold; font-size:15px;'>(Päänäyttämö)</span></div>")
            
            # Päänäyttämöt listataan AINA ensin
...
def yhdista_kulttuuridata(paikat, pvm_iso: str):
    for p in paikat:
        hakusana = p.get("hakusana")
        tapahtumat = hae_tapahtumat_api(hakusana, pvm_iso) if hakusana else []
...
def venue_html(paikat):
    ...
    for p in paikat:
        ...
        <div class='eventline'>
            ...
            <span class='venue-address'>Max Kapasiteetti (talon yhteensä): <b>{p.get('kap','')}</b></span><br>
...
def render_dashboard():
    ...
    with tab1:
        kulttuuri = [
            {"nimi": "Helsingin Kaupunginteatteri (HKT)", "kap": "Suuri: ~900 hlö", "hakusana": "kaupunginteatteri", "huomio": "Yleensä ti-su klo 19", "linkki": "https://hkt.fi/kalenteri/"},
            {"nimi": "Kansallisooppera ja baletti", "kap": "Suuri: ~1300 hlö", "hakusana": "kansallisooppera", "huomio": "Yleensä ohjelmaa klo 19", "linkki": "https://oopperabaletti.fi/ohjelmisto-ja-liput/"},
            {"nimi": "Musiikkitalo", "kap": "Suuri: 1704 hlö", "hakusana": "musiikkitalo", "huomio": "Konsertit usein klo 19", "linkki": "https://musiikkitalo.fi/tapahtumakalenteri/"}
        ]
