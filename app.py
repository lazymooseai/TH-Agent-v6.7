...
def hae_tapahtumat_api(hakusana: str, pvm_iso: str) -> list:
    ...
    # Haetaan tapahtumat laajalla teksti-haulla, jotta myös alisalien tapahtumat löytyvät
    params = {
        "text": hakusana,
        "start": pvm_iso,
        "end": pvm_iso,
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            for t in r.json().get("data", []):
                ...
                # Bouncer-logiikka: Varmistetaan, että tapahtuma on tänään SUOMEN ajassa
                ...
                # Suodatetaan roskadata (esim. koko päivän kestävät näyttelyt joilla ei ole kellonaikaa)
                if alku_klo == "00:00" and not loppu_klo: continue
                
                # Vältetään identtiset duplikaatit
                avain = f"{nimi}-{alku_klo}"
                if avain in nimet_seen: continue
                nimet_seen.add(avain)
...
def hae_liiga_pvm(pvm_iso: str):
    ...
    # Koska voi olla kevät ja pudotuspelit, haetaan rinnakkain runkosarja ja playoffs!
