...
def hae_juna_asemat():
    ...
    try:
        ...
        if r.status_code == 200:
            for s in r.json(): asemat[s["stationShortCode"]] = s["stationName"].replace(" asema", "")
    except: pass
    return asemat
...
def get_trains(asema_nimi: str):
    ...
    tulos = []
    try:
        r = requests.get(f"https://rata.digitraffic.fi/api/v1/live-trains/station/{koodi}?arriving_trains=40&include_nonstopping=false&train_categories=Long-distance", timeout=8)
        if r.status_code == 200:
            for juna in r.json():
                        ...
                        if raaka:
                            try:
                                ...
                                if aika_hki >= nykyhetki - datetime.timedelta(minutes=5):
                                    tulos.append({"train": nimi, "origin": asemat_dict.get(lahto_koodi, lahto_koodi), "time": aika_hki.strftime("%H:%M"), "delay": rivi.get("differenceInMinutes", 0), "dt": aika_hki})
                            except: pass
                        break
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            for lento in saapuvat:
                ...
                tulos.append({"flight": lento.get("fltnr", "??"), "origin": lento.get("route_n_1", "Tuntematon"), "time": aika_r[11:16] if "T" in aika_r else aika_r[:5], "type": f"Laajarunko ({actype})" if wb else f"Kapearunko ({actype})", "wb": wb, "status": status or "Odottaa"})
...
def hae_tapahtumat_api(kohde: dict, pvm_iso: str) -> list:
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            tulos_pieni = []
            ajat_seen = set()
            pieni_keywords = ["pieni", "almin", "almisali", "studio pasila", "lilla teatern", "arena", "camerata", "sonore", "black box", "organo", "paavo", "klubi", "lämpiö", "aula", "kahvila", "ravintola", "foajee", "parvi"]
            ...
            for t in r.json().get("data", []):
                ...
                # Suodatin: Oikea päivä
                if alku_dt.strftime("%Y-%m-%d") != pvm_iso: continue
                    
                # Suodatin: Näyttelyt pois (> 14h)
                if loppu_dt:
                ...
                # ==== TIUKKA OVIKONTROLLI: Estetään harhaosumat ====
                loc_name_l = loc_name.lower()
                osoite_l = osoite.lower()
                
                if kohde["api_text"] == "ooppera":
                    # Kelpuutetaan jos nimessä on ooppera/almin TAI jos katuosoite on oikea
                    if not ("ooppera" in loc_name_l or "almin" in loc_name_l or "helsinginkatu" in osoite_l): 
                        continue
                elif kohde["api_text"] == "musiikkitalo":
                    if "musiikkitalo" not in loc_name_l and "mannerheimintie 13" not in osoite_l: 
                        continue
                elif kohde["api_text"] == "kaupunginteatteri":
                    if not any(kw in loc_name_l for kw in ["kaupunginteatteri", "arena", "pasila", "lilla teatern", "eläintarhan"]): 
                        continue
                # ====================================================
                ...
                if is_pieni:
                    tulos_pieni.append(f"<div style='color:#999; font-size:15px; margin-bottom:8px; line-height:1.2;'>▷ Klo {aika_naytto}: {nimi}<br><span style='color:#777; font-size:14px;'>&nbsp;&nbsp;&nbsp;&nbsp;📍 <i>{sali_info}</i></span></div>")
                else:
                    tulos_paa.append(f"<div style='color:#d0d0d0; font-size:18px; margin-bottom:12px; line-height:1.3; border-left: 2px solid #5bc0de; padding-left: 8px;'>► <b>Klo {aika_naytto}</b>: {nimi}<br><span style='color:#ffeb3b; font-weight:bold; font-size:15px;'>📍 {sali_info}</span></div>")
...
def yhdista_kulttuuridata(paikat, pvm_iso: str):
    for p in paikat:
        ...
        if tapahtumat:
            ...
            p["lopetus_html"] = f"<span class='no-event'>Ei havaittuja esityksiä API:ssa.</span><br><span style='color:#777; font-size:15px;'>ℹ️ {p.get('huomio','')}</span>"
    return paikat
...
def yhdista_urheiludata(paikat, pvm_iso: str):
    ...
    for p in paikat:
        ...
        if "hifk" in nimi_lower:
            tapahtumat = [f"<div style='color:#d0d0d0; font-size:18px; margin-bottom:6px;'>► <b>Klo {peli['aika']}</b>: {peli['koti']} - {peli['vieras']}</div>" for peli in pelit if "hifk" in peli["koti"].lower()]
        elif "espoo" in nimi_lower:
            tapahtumat = [f"<div style='color:#d0d0d0; font-size:18px; margin-bottom:6px;'>► <b>Klo {peli['aika']}</b>: {peli['koti']} - {peli['vieras']}</div>" for peli in pelit if "espoo" in peli["koti"].lower()]
            ...
            continue
        if tapahtumat:
            p["lopetus_html"] = f"<div style='margin-top:10px;'><span class='live-event'>KOTIOTTELU TÄNÄÄN:</span><br>{''.join(tapahtumat)}<span style='color:#ccc;font-size:15px;display:block;margin-top:4px;'>ℹ️ Yleisö purkautuu n. 2,5h aloituksesta.</span></div>"
        else:
...
def venue_html(paikat):
    html = ""
    for p in paikat:
        html += f"<div class='eventline'><span class='venue-name'>{p.get('nimi','')}</span><br><span class='venue-address'>Max Kapasiteetti: <b>{p.get('kap','')}</b></span><br>{p.get('lopetus_html', '')}<br>"
        if 'linkki' in p: html += f"<a href='{p['linkki']}' class='taksi-link' target='_blank' style='font-size:14px;'>Viralliset sivut & Liput</a>"
        html += "</div>"
    return html
...
def render_dashboard():
    ...
    st.markdown(f"<div class='header-container'><div><div class='app-title'>🚕 TH Taktinen Tutka</div><div class='time-display'>{klo} <span style='font-size:16px;color:#888;'>{paiva}</span></div></div><div style='text-align:right;'><a href='https://www.ilmatieteenlaitos.fi/sade-ja-pilvialueet?area=etela-suomi' class='taksi-link' target='_blank'>Säätutka</a> | <a href='https://liikennetilanne.fintraffic.fi/' class='taksi-link' target='_blank'>Liikenne</a></div></div>", unsafe_allow_html=True)
    ...
    else: juna_html += "Ei dataa tai ei saapuvia kaukojunia lähiaikoina."
    ...
    with tab1:
        # Paluu tekstihakuun! Tällä Ooppera löytyy.
        kulttuuri = [
            {"nimi": "Helsingin Kaupunginteatteri (HKT)", "kap": "Suuri: ~900 hlö", "api_text": "kaupunginteatteri", "huomio": "Yleensä ti-su klo 19", "linkki": "https://hkt.fi/kalenteri/"},
            {"nimi": "Kansallisooppera ja baletti", "kap": "Suuri: ~1300 hlö", "api_text": "ooppera", "huomio": "Yleensä ohjelmaa klo 19", "linkki": "https://oopperabaletti.fi/ohjelmisto-ja-liput/"},
            {"nimi": "Musiikkitalo", "kap": "Suuri: 1704 hlö", "api_text": "musiikkitalo", "huomio": "Konsertit usein klo 19", "linkki": "https://musiikkitalo.fi/tapahtumakalenteri/"}
        ]
...
if st.session_state.authenticated: render_dashboard()
