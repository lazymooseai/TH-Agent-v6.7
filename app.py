...
def hae_tapahtumat_api(hakusana: str, pvm_iso: str) -> list:
    ...
    params = {
        ...
        "super_event_type": "none", # Estää viikkoja kestävät ylätason "kattotapahtumat"
        "language": "fi",
        "sort": "start_time"
    }
    ...
    try:
        ...
        if r.status_code == 200:
            tulos = []
            ajat_seen = set() # Tähän tallennetaan kellonajat, jotta tuplabuukkaukset blokataan
            ...
            for t in r.json().get("data", []):
                ...
                # SÄÄNTÖ 1: Tapahtuman TÄYTYY alkaa valittuna päivänä Suomen ajassa.
                if alku_dt.strftime("%Y-%m-%d") != pvm_iso:
                    continue
                    
                # SÄÄNTÖ 2: Suodatetaan pois yli 14h kestävät tapahtumat (pysyvät näyttelyt jne.)
                if loppu_dt:
                ...
                # Ohitetaan epämääräiset 00:00 koko päivän ohjelmat
                if alku_klo == "00:00" and not loppu_klo: continue
                
                # SÄÄNTÖ 3: Vain yksi esitys per kellonaika. Poistaa saman esityksen kielikopiot yms.
                if alku_klo in ajat_seen:
                    continue
                ajat_seen.add(alku_klo)
...
def render_dashboard():
    ...
    with tab1:
        kulttuuri = [
            {"nimi": "Helsingin Kaupunginteatteri (HKT)", "kap": "947 hlö", "hakusana": "kaupunginteatteri", "huomio": "Yleensä ti-su klo 19", "linkki": "https://hkt.fi/kalenteri/"},
            {"nimi": "Kansallisooppera ja baletti", "kap": "1 700 hlö", "hakusana": "kansallisooppera", "huomio": "Yleensä ohjelmaa klo 19", "linkki": "https://oopperabaletti.fi/ohjelmisto-ja-liput/"},
            {"nimi": "Musiikkitalo", "kap": "1 704 hlö", "hakusana": "musiikkitalo", "huomio": "Konsertit usein klo 19", "linkki": "https://musiikkitalo.fi/tapahtumakalenteri/"}
        ]
