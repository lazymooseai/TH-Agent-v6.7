                        ...
                        try:
                            ...
                            if aika_hki >= nykyhetki - datetime.timedelta(minutes=5):
                                ...
                                tulos.append({
                                    ...
                                    "time": aika_hki.strftime("%H:%M"), "delay": viive, "dt": aika_hki
                                })
                        except:
                            pass
                    break
    ...
    try:
        ...
        for taulu in soup.find_all("table"):
            for rivi in taulu.find_all("tr"):
                ...
                if "alus" in rivi_teksti or "laiva" in rivi_teksti: continue
                ...
                term = "Länsiterminaali T2" if ("t2" in rivi_teksti or "finlandia" in nimi.lower()) else "Olympia / Katajanokka"
                
                laivat.append({"ship": nimi, "terminal": term, "time": aika_str, "pax": pax})
        return laivat[:5] if laivat else []
    except:
        return []
...
def parse_hel_api_time(time_str):
    ...
    try:
        ...
        return dt_utc.astimezone(ZoneInfo("Europe/Helsinki")).strftime("%H:%M")
    except:
        return None

@st.cache_data(ttl=86400, show_spinner=False)
def hae_paikka_id(hakusana: str) -> str:
    # Haetaan luotettava ID.
    try:
        r = requests.get("https://api.hel.fi/linkedevents/v1/place/", params={"text": hakusana, "format": "json"}, timeout=5)
...
def hae_tapahtumat_api(paikka_id: str, pvm_iso: str) -> list:
    if not paikka_id: return []
    params = {
        "location": paikka_id,
        "start": pvm_iso,
        "end": pvm_iso,
        "super_event_type": "none", # Estää ylätason viikkotapahtumien tulostumisen
        "language": "fi",
        "sort": "start_time"
    }
    try:
        ...
        if r.status_code == 200:
            ...
            for t in r.json().get("data", []):
                ...
                # Jätetään huomiotta koko päivän tapahtumat joilla ei ole kellonaikaa
                if not alku_klo or alku_klo == "00:00": continue 
                
                aika_naytto = f"{alku_klo} - {loppu_klo}" if loppu_klo else f"{alku_klo} (alkaa)"
                tulos.append(f"<div class='event-list-item'>► <b>Klo {aika_naytto}</b>: {nimi}</div>")
            return tulos
    except: pass
    return []
...
def yhdista_kulttuuridata(paikat, pvm_iso: str):
    for p in paikat:
        paikka_id = p.get("hardcoded_id") or (hae_paikka_id(p["hakusanat"][0]) if p.get("hakusanat") else None)
        tapahtumat = hae_tapahtumat_api(paikka_id, pvm_iso) if paikka_id else []
        
        if tapahtumat:
            p["lopetus_html"] = f"<div style='margin-top:10px;'><span class='live-event'>ESITYKSET TÄNÄÄN:</span><br>{''.join(tapahtumat)}</div>"
        else:
            p["lopetus_html"] = f"<span class='no-event'>Ei esityksiä kaupungin API:ssa. (Tyypillisesti: {p.get('huomio','')})</span>"
    return paikat

@st.cache_data(ttl=3600, show_spinner=False)
def hae_liiga_pvm(pvm_iso: str):
    # Lasketaan kausi dynaamisesti (esim. 2025-2026)
    dt_obj = datetime.datetime.strptime(pvm_iso, "%Y-%m-%d")
    kausi_alku = dt_obj.year if dt_obj.month > 7 else dt_obj.year - 1
    
    # Ei "tournament=runkosarja" rajoitusta, jotta myös pudotuspelit näkyvät!
    url = f"https://liiga.fi/api/v2/games?season={kausi_alku}-{kausi_alku+1}"
    try:
        r = requests.get(url, headers={"User-Agent": "Mozilla/5.0"}, timeout=8)
        if r.status_code == 200:
            pelit = []
...
def yhdista_urheiludata(paikat, pvm_iso: str):
    ...
    for p in paikat:
        ...
        # Täsmäytetään Liigan API-nimet paikkoihin
        if "hifk" in nimi_lower:
            tapahtumat = [f"<div class='event-list-item'>► <b>Klo {peli['aika']}</b>: {peli['koti']} - {peli['vieras']}</div>" 
            ...
            p["lopetus_html"] = "<span class='no-event'>Mestis ei tuettu suorassa haussa. <a href='https://jokerit.fi/ottelut' target='_blank' style='color:#5bc0de;'>Katso sivuilta →</a></span>"
            continue

        if tapahtumat:
            p["lopetus_html"] = (
                f"<div style='margin-top:10px;'><span class='live-event'>KOTIOTTELU:</span><br>{''.join(tapahtumat)}"
                f"<span style='color:#ccc;font-size:15px;display:block;margin-top:4px;'>ℹ️ Yleisö purkautuu n. 2h 30min aloituksesta.</span></div>"
            )
        else:
...
def venue_html(paikat):
    ...
    for p in paikat:
        ...
        <div class='eventline'>
            ...
            <span class='venue-address'>Kapasiteetti: <b>{p.get('kap','')}</b></span><br>
            {p.get('lopetus_html', '')}<br>
            {"<a href='"+p['linkki']+"' class='taksi-link' target='_blank' style='font-size:14px;'>Viralliset sivut</a>" if 'linkki' in p else ""}
        </div>
        """
    return html
...
def render_dashboard():
    ...
    tab1, tab2, tab3 = st.tabs(["Kulttuuri (API)", "Urheilu (Liiga)", "Messut & Musiikki (Staattinen)"])

    with tab1:
        # Kulttuuripaikkojen tiedot (Käytetään hardcoded_id luotettavuuden takaamiseksi Hel.fi API:ssa)
        kulttuuri = [
            {"nimi": "Helsingin Kaupunginteatteri (HKT)", "kap": "947 hlö", "hakusanat": ["kaupunginteatteri"], "hardcoded_id": "tprek:28473", "huomio": "Yleensä ti-su klo 19", "linkki": "https://hkt.fi/kalenteri/"},
            {"nimi": "Kansallisooppera ja baletti", "kap": "1 700 hlö", "hakusanat": ["kansallisooppera"], "hardcoded_id": "tprek:8744", "huomio": "Yleensä klo 19", "linkki": "https://oopperabaletti.fi/"},
            {"nimi": "Musiikkitalo", "kap": "1 704 hlö", "hakusanat": ["musiikkitalo"], "hardcoded_id": "tprek:18874", "huomio": "Konsertit klo 19", "linkki": "https://musiikkitalo.fi/"}
        ]
